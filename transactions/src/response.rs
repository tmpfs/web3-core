...
use crate::types::{Address, Bloom, Bytes, Log, H256, U256, U64};
...
impl Decodable for Transaction {
    fn decode(rlp: &rlp::Rlp) -> Result<Self, DecoderError> {
        ...
        let rest = rlp::Rlp::new(
            rlp.as_raw()
                .get(1..)
                .ok_or(DecoderError::Custom("no transaction payload"))?,
        );
...
mod tests {
    use crate::eip2930::AccessListItem;
    ...
    fn decode_rlp_london_goerli() {
        ...
        let decoded_transaction =
            Transaction::decode(&rlp::Rlp::new(&rlp_bytes)).unwrap();
