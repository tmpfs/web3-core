...
use syn::{
    parse::Error, spanned::Spanned as _, AttrStyle, Data, DeriveInput, Expr,
    Fields, GenericArgument, Lit, NestedMeta, PathArguments, Type,
};
...
pub const EIP712_DOMAIN_TYPE_HASH_WITH_SALT: [u8; 32] = [
    216, 124, 214, 239, 121, 212, 226, 185, 94, 21, 206, 138, 191, 115, 45,
    181, 30, 199, 113, 241, 202, 46, 220, 207, 34, 164, 108, 114, 154, 197,
    100, 114,
];
...
impl<T: Eip712 + Clone> Eip712 for EIP712WithDomain<T> {
    ...
    fn type_hash() -> Result<[u8; 32], Self::Error> {
        let type_hash =
            T::type_hash().map_err(|e| Self::Error::Inner(e.to_string()))?;
        Ok(type_hash)
    }
    ...
    fn try_from(input: &syn::DeriveInput) -> Result<EIP712Domain, Self::Error> {
                ...
                if let Ok(syn::Meta::List(meta)) = attribute.parse_meta() {
                    if meta.path.is_ident("eip712") {
                        ...
                        if domain.name == String::default() {
                            return Err(Error::new(
                                meta.path.span(),
                                "missing required domain attribute: 'name'"
                                    .to_string(),
                            )
                            .to_compile_error());
                        }
                        if domain.version == String::default() {
                            return Err(Error::new(
                                meta.path.span(),
                                "missing required domain attribute: 'version'"
                                    .to_string(),
                            )
                            .to_compile_error());
                        }
                        if domain.chain_id == U256::default() {
                            return Err(Error::new(
                                meta.path.span(),
                                "missing required domain attribute: 'chain_id'"
                                    .to_string(),
                            )
                            .to_compile_error());
                        }
        ...
        if !found_eip712_attribute {
            return Err(Error::new_spanned(
                input,
                "missing required derive attribute: '#[eip712( ... )]'"
                    .to_string(),
            )
            .to_compile_error());
        }
                ...
                return match ident.as_str() {
                    ...
                    s => parse_int_param_type(s).ok_or_else(|| {
                        Error::new(
                            ty.span(),
                            format!(
                                "Failed to derive proper ABI from field: {})",
                                s
                            ),
                        )
                        .to_compile_error()
...
pub fn parse_fields(
    ast: &DeriveInput,
) -> Result<Vec<(String, ParamType)>, TokenStream> {
    ...
    for f in named_fields.named.iter() {
        let field_name = f
            ...
            .ok_or_else(|| {
                Error::new(named_fields.span(), "fields must be named")
                    .to_compile_error()
            })?;
...
pub fn encode_eip712_type(token: Token) -> Token {
    match token {
        ...
        Token::Array(tokens) => {
            Token::Uint(U256::from(keccak256(abi::encode(
                &tokens
                    .into_iter()
