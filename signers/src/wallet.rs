...
use std::fmt;
use thiserror::Error;
use web3_hash_utils::{prefix_message, to_checksum};
...
impl<S: Sign> Wallet<S> {
    ...
    pub async fn sign_message<M: Send + Sync + AsRef<[u8]>>(
        ...
        let message = prefix_message(message);
        Ok(self.sign_data(&message, false).await?)
    }
    ...
    pub async fn sign_transaction(
        ...
        match chain_id {
            Some(id) => {
                ...
                Ok(self.sign_data(tx.rlp(), true).await?)
            }
            None => {
                // in the case we don't have a chain_id,
                // let's use the signer chain id instead
                let mut tx_with_chain = tx.clone();
                tx_with_chain.set_chain_id(self.chain_id);
                Ok(self.sign_data(tx_with_chain.rlp(), true).await?)
            }
        }
    }
    ...
    */
    ...
    /// Sign the bytes for RLP encoded data or a raw message.
    pub async fn sign_data<D>(
        &self,
        data: D,
        eip155: bool,
    ) -> Result<Signature, WalletError>
