...
use serde::{
    ser::Error as SerializationError, Deserialize, Deserializer, Serialize,
    Serializer,
};
...
impl Serialize for NameOrAddress {
        ...
        S: Serializer,
    {
        match self {
            ...
            NameOrAddress::Name(name) => Err(SerializationError::custom(
                format!("cannot serialize ENS name {}, must be address", name),
            )),
        }
    }
}
